...
class HDF5Dataset(Dataset):
    """
    A PyTorch Dataset to load data from an HDF5 file.
    """
    def __init__(self, hdf5_path):
        ...
        """
        self.hdf5_data = HDF5Read(hdf5_path)

    def __len__(self):
        """
        Return the total number of samples in the dataset.
        """
        return self.hdf5_data.get_size()

    def __getitem__(self, idx):
        """
        Retrieve the ith sample from the dataset.
        :param idx: Index of the sample to retrieve.
        :return: A tuple (data, label) where data is the data tensor and label is its corresponding label.
        """
        data, label = self.hdf5_data.get_datum(idx)
